...
Open models can be used with Copilot CLI through Lychee, enabling you to use models such as `qwen3.5`, `glm-5.1:cloud`, `kimi-k2.5:cloud`.
...
```shell
lychee launch copilot
```

### Run directly with a model

```shell
lychee launch copilot --model kimi-k2.5:cloud
```
...
```shell
lychee launch copilot --model kimi-k2.5:cloud --yes -- -p "how does this repository work?"
```
...
Copilot CLI connects to Lychee using the OpenAI-compatible API via environment variables.
...
**Note:** Copilot requires a large context window. We recommend at least 64k tokens. See the [context length documentation](/context-length) for how to adjust context length in Lychee.
