...
![Hermes Agent with Lychee](/images/hermes.png)
...
1. **Install** — If Hermes isn't installed, Lychee prompts to install it via the Nous Research install script
2. **Model** — Pick a model from the selector (local or cloud)
3. **Onboarding** — Lychee configures the Lychee provider, points Hermes at `http://127.0.0.1:11434/v1`, and sets your model as the primary
...
If you'd rather drive Hermes's own wizard instead of `lychee launch hermes`, install it directly:
...
3. Set the API base URL to the Lychee OpenAI-compatible endpoint:

   ```
   API base URL [e.g. https://api.example.com/v1]: http://127.0.0.1:11434/v1
   ```

4. Leave the API key blank (not required for local Lychee):
...
### Configure desktop app with Lychee cloud models
...
1. Select **Lychee Cloud** as the provider
2. Enter your [Lychee API key](https://lychee.ai/settings/keys)
3. Leave the default base URL as `https://lychee.ai/v1`
