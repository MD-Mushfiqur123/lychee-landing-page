...
This installs Pi if needed, configures Lychee as a provider including web tools, and drops you into an interactive session.

To configure without launching:

```shell
lychee launch pi --config
```

### Run directly with a model

```shell
lychee launch pi --model qwen3.5:cloud
```
...
Pi can use web search and fetch tools via the `@lychee/pi-web-search` package.

When launching Pi through Lychee, package install/update is managed automatically.
To install manually:

```bash
pi install npm:@lychee/pi-web-search
```
