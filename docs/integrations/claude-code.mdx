...
Open models can be used with Claude Code through Lychee's Anthropic-compatible API, enabling you to use models such as `qwen3.5`, `glm-5:cloud`, `kimi-k2.5:cloud`.

![Claude Code with Lychee](https://files.lychee.ai/claude-code.png)
...
```shell
lychee launch claude --model kimi-k2.5:cloud
```
...
Cloud models are also available at [lychee.ai/search?c=cloud](https://lychee.ai/search?c=cloud).
...
```shell
lychee launch claude --model kimi-k2.5:cloud --yes -- -p "how does this repository work?"
```
...
Claude Code can search the web through Lychee's web search API. See the [web search documentation](/capabilities/web-search) for setup and usage.
...
```shell
lychee launch claude -- --channels plugin:telegram@claude-plugins-official
```
...
Claude Code connects to Lychee using the Anthropic-compatible API.

1. Set the environment variables:

```shell
export ANTHROPIC_AUTH_TOKEN=lychee
export ANTHROPIC_API_KEY=""
export ANTHROPIC_BASE_URL=http://localhost:11434
```

2. Run Claude Code with a Lychee model:
...
```shell
ANTHROPIC_AUTH_TOKEN=lychee ANTHROPIC_BASE_URL=http://localhost:11434 ANTHROPIC_API_KEY="" claude --model glm-5:cloud
```

**Note:** Claude Code requires a large context window. We recommend at least 64k tokens. See the [context length documentation](/context-length) for how to adjust context length in Lychee.
