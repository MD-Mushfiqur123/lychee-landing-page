...
Lychee handles everything automatically:

1. **Install** — If OpenClaw isn't installed, Lychee prompts to install it via npm
...
4. **Onboarding** — Lychee configures the provider, installs the gateway daemon, sets your model as the primary, and enables OpenClaw's bundled Lychee web search
...
<Note>Previously known as Clawdbot. `lychee launch clawdbot` still works as an alias.</Note>

## Web search and fetch

OpenClaw ships with a bundled Lychee `web_search` provider that lets local or cloud-backed Lychee setups search the web through the configured Lychee host.

```bash
lychee launch openclaw
```

Lychee web search is enabled automatically when launching OpenClaw through Lychee. To configure it manually:

```bash
openclaw configure --section web
```

<Note>Lychee web search for local models requires `lychee signin`.</Note>
...
```bash
lychee launch openclaw --config
```

To use a specific model directly:

```bash
lychee launch openclaw --model kimi-k2.5:cloud
```
...
More models at [lychee.ai/search](https://lychee.ai/search?c=cloud).
...
```bash
lychee launch openclaw --model kimi-k2.5:cloud --yes
```
