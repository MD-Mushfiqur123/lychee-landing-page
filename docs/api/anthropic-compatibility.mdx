...
Lychee provides compatibility with the [Anthropic Messages API](https://docs.anthropic.com/en/api/messages) to help connect existing applications to Lychee, including tools like Claude Code.

## Usage

### Environment variables

To use Lychee with tools that expect the Anthropic API (like Claude Code), set these environment variables:

```shell
export ANTHROPIC_AUTH_TOKEN=lychee  # required but ignored
...
client = anthropic.Anthropic(
    ...
    api_key='lychee',  # required but ignored
)
...
const anthropic = new Anthropic({
  ...
  apiKey: "lychee", // required but ignored
});
...
[Claude Code](https://code.claude.com/docs/en/overview) can be configured to use Lychee as its backend. 
...
```shell
ANTHROPIC_AUTH_TOKEN=lychee ANTHROPIC_BASE_URL=http://localhost:11434 claude --model qwen3-coder
```
...
Then run Claude Code with any Lychee model:
...
Lychee supports both local and cloud models.
...
For tooling that relies on default Anthropic model names such as `claude-3-5-sonnet`, use `lychee cp` to copy an existing model name:

```shell
lychee cp qwen3-coder claude-3-5-sonnet
```
