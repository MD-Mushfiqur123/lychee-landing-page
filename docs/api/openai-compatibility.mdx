...
Lychee provides compatibility with parts of the [OpenAI API](https://platform.openai.com/docs/api-reference) to help connect existing applications to Lychee.
...
- `owned_by` corresponds to the lychee username, defaulting to `"library"`
...
> Note: Added in Lychee v0.13.3

Lychee supports the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses). Only the non-stateful flavor is supported (i.e., there is no `previous_response_id` or `conversation` support).
...
Before using a model, pull it locally `lychee pull`:
...
For tooling that relies on default OpenAI model names such as `gpt-3.5-turbo`, use `lychee cp` to copy an existing model name to a temporary name:

```shell
lychee cp llama3.2 gpt-3.5-turbo
```
...
Use the `lychee create mymodel` command to create a new model with the updated context size. Call the API with the updated model name:
