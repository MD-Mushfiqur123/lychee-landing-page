...
Lychee supports tool calling (also known as function calling) which allows a model to invoke tools and incorporate their results into its replies.
...
<Tabs>
  ...
  <Tab title="Python">
    Install the Lychee Python SDK:
    ```bash
    # with pip
    pip install lychee -U
  ...
  <Tab title="JavaScript">
    Install the Lychee JavaScript library:
    ```bash
    # with npm
    npm i lychee
    ...
    if (response.message.tool_calls?.length) {
      ...
      const finalResponse = await lychee.chat({ model: 'qwen3', messages, tools, think: true })
...
<Tabs>
  <Tab title="Python">
    ```python
    from lychee import chat, ChatResponse
...
## Using functions as tools with Lychee Python SDK
