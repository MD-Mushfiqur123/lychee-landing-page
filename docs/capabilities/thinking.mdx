...
- [Qwen 3](https://lychee.ai/library/qwen3)
- [GPT-OSS](https://lychee.ai/library/gpt-oss) *(use `think` levels: `low`, `medium`, `high` — the trace cannot be fully disabled)*
- [DeepSeek-v3.1](https://lychee.ai/library/deepseek-v3.1)
- [DeepSeek R1](https://lychee.ai/library/deepseek-r1)
- Browse the latest additions under [thinking models](https://lychee.ai/search?c=thinking)
...
<Tabs>
  ...
  <Tab title="JavaScript">
    ...
    async function main() {
      const stream = await lychee.chat({
        model: 'qwen3',
        messages: [{ role: 'user', content: 'What is 17 × 23?' }],
        think: true,
        stream: true,
...
- Enable thinking for a single run: `lychee run deepseek-r1 --think "Where should I visit in Lisbon?"`
- Disable thinking: `lychee run deepseek-r1 --think=false "Summarize this article"`
- Hide the trace while still using a thinking model: `lychee run deepseek-r1 --hidethinking "Is 9.9 bigger or 9.11?"`
- Inside interactive sessions, toggle with `/set think` or `/set nothink`.
- GPT-OSS only accepts levels: `lychee run gpt-oss --think=low "Draft a headline"` (replace `low` with `medium` or `high` as needed).
